...
public class AppListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//窗口的设置 ，没有标题窗口样式，注意该方法必须在setContentView之前
		requestWindowFeature(Window.FEATURE_NO_TITLE);
		setContentView(R.layout.activity_app_list);
		//引用列表视图
		GridView appListView=(GridView)findViewById(R.id.app_lstView);
		...
		for(int i=0;i<12;i++){
			...
			map.put("icon", R.drawable.emoji_054+i);//属性1
			map.put("name", "应用"+(i+1));//属性2
			map.put("size", 5);//属性3
			data.add(map);
		}
		/**3. 准备适配器  
